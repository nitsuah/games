...
import { MIN_ALIVE_TIME } from '@/lib/asteroid/_comp/config';
  ...
  useFrame(() => {
    ...
    setTargets((prevTargets) => {
      ...
      for (let i = 0; i < updatedTargets.length; i++) {
        ...
        const sphereA = new THREE.Sphere(new THREE.Vector3(targetA.x, targetA.y, targetA.z), targetA.size / 2);
        ...
        for (let j = i + 1; j < updatedTargets.length; j++) {
          ...
          const sphereB = new THREE.Sphere(new THREE.Vector3(targetB.x, targetB.y, targetB.z), targetB.size / 2);

          if (sphereA.intersectsSphere(sphereB)) {
            updatedTargets = updatedTargets.filter((t) => t.id !== targetA.id && t.id !== targetB.id);
